...
;(function() {
  ...
  function _fmtToFT_IN_FRAC(base, denom, sfx) {
    ...
    return function (valueIn) {
      ...
      const signStr = valueIn < 0 ? '-' : '';
      let numerator = Math.round(denom * (inches - wholeInches));

      if (numerator === 0) {
        return signStr + Math.abs(feet) + sfx[0] + Math.abs(wholeInches) + sfx[1];
      }
      while (numerator % 2 === 0 && denom % 2 === 0) {
        numerator /= 2;
        denom /= 2;
      }
      return signStr + Math.abs(feet) + sfx[0] + Math.abs(wholeInches) + ' ' + Math.abs(numerator) + '/' + denom + sfx[1];
    };
  }
